...
**Primary free line (homepage):** *Revenue Intel Briefing* — user submits ICP + market/niche → personalized briefing emailed. ESP tag: `revenue_intel_briefing`. API: `/api/revenue-intel-briefing.php`.
...
Kindle author page · website footer · email signature · course intros · LinkedIn headline — **same line**.
...
| Field | Spec |
|---|---|
| **Who** | Growth operators, solopreneurs, agency owners, AI builders, fractional CMOs |
| **Pain** | Generic AI slop; products without persuasion layer; manual copy grind; funnels that leak |
| **Desire** | Autonomous systems that research, write, and convert — with classical DR psychology underneath |
| **Awareness** | Solution-aware to product-aware (Schwartz 3–4) |
| **Warm segment** | Readers of *AI Product Design Mastery* — already trust Samuel on AI product craft |
| **Offer ladder** | Kindle (live + pipeline) → course/toolkit → consulting / $10K website → agency systems |

---

## Visual & verbal signature (starter)
...
## Runtime & API (operator setup)

Samuel runs The Architect with:
...
```bash
cp .env.example .env
# Add keys locally — NEVER commit .env
```

**GitHub Actions factory:** repo secret `ANTHROPIC_API_KEY` only (Claude runs factory today).
...
## Kindle Author Central checklist
