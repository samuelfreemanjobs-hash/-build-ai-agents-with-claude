...
function registerBusinessRoutes(app, { supabase, genAI, resend, sendSlackNotification, requireSupabase }) {
  ...
  app.post('/api/proposals/generate', async (req, res) => {
    ...
    try {
      ...
      const { data, error: insertErr } = await supabase.from('proposals').insert([proposal]).select();
      if (insertErr) throw insertErr;

      await supabase.from('leads').update({ stage: 'Proposal Active', last_activity: new Date().toISOString() }).eq('id', leadId);
  ...
  app.put('/api/proposals/:id', async (req, res) => {
    if (!requireSupabase(res)) return;
    const { data, error } = await supabase.from('proposals').update(req.body).eq('id', req.params.id).select();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data[0]);
  });
  ...
  app.post('/api/diagnostics/generate', async (req, res) => {
    ...
    try {
      ...
      const { data, error: insertErr } = await supabase.from('diagnostics').insert([diagnostic]).select();
      if (insertErr) throw insertErr;

      await supabase.from('leads').update({ stage: 'Diagnostic Ready', last_activity: new Date().toISOString() }).eq('id', leadId);
  ...
  app.post('/api/leads/:id/convert', async (req, res) => {
    ...
    try {
      ...
      await supabase.from('leads').update({ stage: 'Closed Won / Client', last_activity: new Date().toISOString() }).eq('id', id);
  ...
  app.put('/api/projects/:id', async (req, res) => {
    ...
    const { data, error } = await supabase.from('projects').update(updates).eq('id', req.params.id).select();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data[0]);
  });
  ...
  app.put('/api/tasks/:id', async (req, res) => {
    ...
    const { data, error } = await supabase.from('tasks').update(updates).eq('id', req.params.id).select();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data[0]);
  });
  ...
  app.post('/api/capture', async (req, res) => {
    try {
      ...
      if (requireSupabase(res) === false) return;

      const lead = await createCapturedLead(supabase, { company, email, industry, challenge });
...
async function createCapturedLead(supabase, { company, email, industry, challenge }) {
  const lead = {
    id: crypto.randomUUID(),
    company,
