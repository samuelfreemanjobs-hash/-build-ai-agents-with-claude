...
function computeLaunchReadiness({ leads = [], outreachCount = 0, storeMode = 'local' } = {}) {
  const env = process.env;
  const isSet = (v) => v && !/your-|xxx|placeholder/i.test(v);

  const items = [
    { id: 'pipeline', label: 'Pipeline loaded (10+ leads)', done: leads.length >= 10 },
    { id: 'hot', label: 'HOT leads scored', done: leads.some(l => (l.score || 0) >= 90) },
    { id: 'gemini', label: 'Gemini AI connected', done: !!env.GEMINI_API_KEY },
    { id: 'resend', label: 'Resend email ready', done: !!env.RESEND_API_KEY },
    { id: 'booking', label: 'Cal.com booking link', done: isSet(env.BOOKING_URL) },
    { id: 'supabase', label: 'Supabase production DB', done: storeMode === 'supabase' },
    { id: 'deploy', label: 'PUBLIC_URL for webhooks', done: isSet(env.PUBLIC_URL) },
    { id: 'slack', label: 'Slack alerts (optional)', done: isSet(env.SLACK_WEBHOOK) },
    { id: 'outreach', label: 'Day-1 sends (4 HOT emails)', done: outreachCount >= 4 },
    { id: 'spark', label: 'Spark webhook secret set', done: isSet(env.SPARK_WEBHOOK_SECRET) }
  ];
...
function buildDailyBriefing(metrics, leads, tasks, extras = {}) {
  ...
  return {
    ...
    launch: computeLaunchReadiness({
      leads,
      outreachCount: extras.outreachCount || 0,
      storeMode: extras.storeMode || 'local'
    })
  };
}
...
module.exports = { computeBusinessMetrics, buildDailyBriefing, computeLaunchReadiness };
